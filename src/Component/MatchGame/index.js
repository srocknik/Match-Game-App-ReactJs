...
const navList = [
  {
    ...
    id: 'kmsacosocnoascjonsac',
  },
]

class MatchGame extends Component {
  state = {
    imageList: initialImagesList,
    defaultTab: 'FRUIT',
    count: 0,
    timer: 60,
    randomIndex: 0,
  ...
  getMatched = id => {
    ...
    if (id === initialImagesList[randomIndex].id) {
      this.setState(prevState => ({
        count: prevState.count + 1,
        randomIndex: Math.ceil(Math.random() * initialImagesList.length - 1),
      }))
    } else {
      this.setState({clickedRight: false})
    }
  }

  onReset = () => {
    this.setState({count: 0, timer: 60, clickedRight: true})
  ...
  render() {
    ...
    const filteredImageList = imageList.filter(
      each => each.category === defaultTab,
    )

    const randomImage = initialImagesList[randomIndex].imageUrl

    return (
      <div className="main-container">
        <ul className="nav-list-container">
          {navList.map(each => (
            <NavBar count={count} timer={timer} navItems={each} key={each.id} />
          ))}
        </ul>
        {clickedRight && timer > 1 ? (
            ...
            <ul className="tab-list-container">
              {tabsList.map(each => (
                <TabItem
                  tabItems={each}
                  key={each.tabId}
                  getTheSelectedImagesList={this.getTheSelectedImagesList}
                  defaultTab={defaultTab}
                />
              ))}
            </ul>
            <ul className="select-image-container">
              {filteredImageList.map(each => (
                <SelectImage
                  ImageListItems={each}
                  key={each.id}
                  getMatched={this.getMatched}
                />
              ))}
            </ul>
          </div>
          ...
          <div className="bg-container">
            ...
            <div className="game-over-container">
              ...
              />
              <h1 className="game-over-heading">Your Score</h1>
              <p className="your-score">{count}</p>
              <button
                type="button"
                className="reset-btn"
